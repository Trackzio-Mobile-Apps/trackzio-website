...
import type { BlogSummary } from "@/lib/content/blogs";
import { blogCategories } from "@/lib/blogConstants";
...
export default function Blog({ articles }: BlogScreenProps) {
  usePageAnalytics("blog", "blog_page_view");
  const blogOfTheWeek = articles.find((a) => a.blogOfTheWeek) ?? articles[0];
  const [activeSplitCategory, setActiveSplitCategory] = useState<string>(blogCategories[0]);
  ...
  }, [activeSplitCategory, showAllCategories, articles]);
  ...
  if (!blogOfTheWeek) {
    return (
      <div className="container-site py-24 text-center text-muted-foreground">
        ...
        <div className="container-site max-w-5xl">
          <motion.div {...fadeUp}>
            ...
            <div className="flex flex-col lg:flex-row gap-6 rounded-xl overflow-hidden">
              <div
                className="lg:w-[38%] bg-card rounded-xl flex flex-col"
              ...
              <Link
                href={activePreview ? `/blog/${activePreview.slug}` : "#"}
                ...
                {activePreview ? (
                  <>
                    ...
                    <div className="p-6 sm:p-8 flex-1 flex flex-col">
                      ...
                      <p className="text-sm text-muted-foreground leading-relaxed">{activePreview.excerpt}</p>
